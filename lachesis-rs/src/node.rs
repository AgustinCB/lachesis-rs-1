...
impl<P: Peer<H>, H: Hashgraph + Clone + fmt::Debug> fmt::Debug for Node<P, H> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        fn print_hashes(f: &mut fmt::Formatter, events: &mut BTreeMap<PeerId, Option<EventHash>>) -> fmt::Result {
            ...
            for peer in events.keys() {
                if let Some(Some(ev)) = events.get(peer) {
                    let s = &String::from_utf8(ev.as_ref().to_vec()).unwrap()[..8];
                    write!(f, "{}  ", s)?;
                } else {
                    write!(f, "          ")?;
                }
        ...
        for peer in network.keys() {
            let s = &String::from_utf8(peer.clone()).unwrap()[..8];
            write!(f, "{}  ", s)?;
        }
        writeln!(f, "")?;
        write!(f, "        ")?;
        ...
        for peer in network.keys() {
            for root in roots.iter() {
                ...
                if e.creator() == peer {
                    let display_hash = &String::from_utf8(root.as_ref().to_vec()).unwrap()[..8];
                    write!(f, "{}  ", display_hash)?;
        ...
        while last_event_per_peer.len() > 0 {
...
impl<P: Peer<H>, H: Hashgraph + Clone + fmt::Debug> Node<P, H> {
    ...
    pub fn respond_message(&self) -> Result<(EventHash, HashgraphWire), Error> {
    ...
    fn get_voters(&self) -> Result<Vec<(usize, EventHash)>, Error> {
        ...
        Ok(state.rounds[next_consensus..max_r].iter()
...
mod tests {
    ...
    fn create_node() -> Node<DummyPeer, BTreeHashgraph> {
